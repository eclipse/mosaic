...
package org.eclipse.mosaic.fed.zeromq.workers;
...
public class NoblockMajordomoWorker 
{
    ...
    void reconnectToBroker()
    {
        ...
        inproc = ctx.createSocket(SocketType.PULL);
        worker.connect(broker);
        inproc.connect(inprocAddr);
        if (verbose)
            log.format("I: connecting to broker at %s\n", broker);
            log.format("I: connecting to inproc at %s\n", inprocAddr);
    ...
    public void getInteraction(ZMQ.Socket inproc){
        ZMsg interaction = ZMsg.recvMsg(inproc);
        ZMQ.Poller inprocItems = ctx.createPoller(1);
        inprocItems.register(inproc, ZMQ.Poller.POLLIN);
